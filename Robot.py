...
class Robot:
    ...
    def init_kalman_filter(self, points):
        ...
        kalman.statePre[:2,0] = np.array(center, dtype=np.float32)
        kalman.statePre[2:,0] = 0
        return kalman
    ...
    def update(self, points, frame_num):
        ...
        center = self.compute_center(points)
        self.kalman.correct(np.array(center, dtype=np.float32))
        self.predicted_center = center

    def predict(self):
        prediction = self.kalman.predict()
        self.predicted_center = (prediction[0][0], prediction[1][0])
